...
<script type="text/javascript">
    ...
    //                         echo "['" . $act->ACTIVITY_ID . "', '" . $act->ACTIVITY_NAME . "', '" . "summer'" . ', new Date(' . $Syear . ',' . $Smonth . ',' . $Sday . '), new Date(' . $Eyear . ',' . $Emonth . ',' . $Eday . '),' . 'null' . ',' . '100' . ',' . 'null' . '],
    //         ';
    //                     }

    //                     // echo "['" . $act->ACTIVITY_ID . "', '" . $act->ACTIVITY_NAME . "', '" . "spring'" . ', new Date(' . $Syear . ',' . $Smonth . ',' . $Sday . '), new Date(' . $Eyear . ',' . $Emonth . ',' . $Eday . '),' . 'null' . ',' . '100' . ',' . 'null' . '],';
    //                 }
        ...
        @php
            foreach ($project_detail->activity as $act) {
                ...
                if ($act->START_DATE && $act->END_DATE) {
                    ...
                    $Emonth = intval(date('m', strtotime($act->END_DATE)));
                    $Eday = intval(date('d', strtotime($act->END_DATE)));
                    echo "['" . $act->ACTIVITY_ID . "', '" . $act->ACTIVITY_NAME . "', " . ' new Date(' . $Syear . ',' . $Smonth . ',' . $Sday . '), new Date(' . $Eyear . ',' . $Emonth . ',' . $Eday . ')' . '],
                    ';
